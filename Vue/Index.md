...
Мапинг (от англ. _mapping_) — это сопоставление **одного значения другому**. В JavaScript чаще всего для этого используется объект `const map = { key1: value1, key2: value2,}`.  
То есть ты "мапишь" `key1` → `value1`, `key2` → `value2`.

Иногда вместо мапинга пишут `if` или `switch`, но мапинг через объект:
✅ компактный  
✅ легко читается  
✅ легко расширять (просто добавляешь новую пару ключ–значение)

#### Пример: мапинг цветов
...
const color = colorMap[primary]; // #007bff
```

#### Пример: мапинг иконок и вывод в `<component :is="">`

![[Pasted image 20250723132240.png]]